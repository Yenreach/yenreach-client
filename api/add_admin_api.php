<?php
    require_once("../../includes_cfcing/initialize.php");
    $return_array = array();
    ...
    if(!empty($post_json)){
        ...
        if($admin->insert("create")){
            ...
            $content .= "<p>";
            $content .=     "In order to activate your Account, please click <a href=\"https://admin.cfcing.org/activate.php?c=".$admin->verify_string;
            $content .=     "&k=".base64_encode($time)."&q=CFCING".md5($time)."\">this link</a>";
            $content .= "</p>";
            ...
            $pdata = [
                    ...
                    'movement' => 'outgoing',
                    'from_name' => 'Yenreach',
                    'from_mail' => 'email@yenreach.com',
                    'recipient_name' => $admin->name,
                    'recipient_mail' => $admin->personal_email,
                    'subject' => $subject,
                    'content' => $content,
                    'reply_name' => 'Yenreach',
                    'reply_mail' => 'email@yenreach.com'
                ];
