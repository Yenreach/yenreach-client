<?php
    require_once("../includes_public/initialize.php");


    $gurl = "fetch_privacy_policy_api.php";
    $privacy_policy = perform_get_curl($gurl);
    if($privacy_policy){
        if($privacy_policy->status == "success"){         
            $privacy_policy = $privacy_policy->data;
        } else {
            redirect_to("blogs.php");
        }
    } else {
            die("Blog Link Broken");
        }


    
?>
<html>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Yenreach Privacy Policy</title>
    <!-- Favicon icon -->
    <link rel="icon" type="image/png" sizes="16x16" href="./images/favicon.png">
   <link href="assets/css/style.css" rel="stylesheet">

    <link
      href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.1/dist/css/bootstrap.min.css"
      rel="stylesheet"
    />
    <link
      rel="stylesheet"
      href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.5.0/font/bootstrap-icons.css"
    />
    <!-- <link href="./css/style.css" rel="stylesheet"> -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Open+Sans:ital,wght@1,300;1,600&display=swap" rel="stylesheet">
<?php include_layout_template("links.php"); ?>

</head>
    <body>
    <?php include_layout_template("header.php"); ?>
        
        <div class="col-12  mt-5">
            <div class="container">
                <div>
                    <?php
                        echo  htmlspecialchars_decode($privacy_policy->content);
                    ?>
                </div>
                <a href='' class="d-flex justify-content-start py-4 align-items-center">
                    <button class="btn bg-success py-2 px-3 text-white" style="font-size: 16px;">I understand</button>
                </a>
            </div>
        </div>
    </body>
    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
</html>
