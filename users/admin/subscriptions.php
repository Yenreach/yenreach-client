        ...
        <table class="table table-borderless datatable">
          ...
          <tbody>
            ...
            <tr>
              ...
              <?php if($results['subscription_type']==4){ ?>
              <td><span class="badge bg-success">Premium</span></td>
            <?php } else if($results['subscription_type']==3){ ?>
              <td><span class="badge bg-info">Gold</span></td>
            <?php } else if($results['subscription_type']==2){ ?>
              <td><span class="badge bg-warning">Silver</span></td>
            <?php } else if($results['subscription_type']==1){ ?>
              <td><span class="badge bg-danger">Free</span></td>
            <?php } ?>
              <td><?php echo date("Y-m-d", strtotime($results['subscription_date'])); ?></td>
              <td><?php echo date("Y-m-d", strtotime($results['expiry_date'])); ?></td>
              <?php if((strtotime($results['expiry_date']) - strtotime('today')) >= 0){
                  $time = strtotime($results['expiry_date']) - strtotime('today');
                  $days = round((($time/60)/60)/24);
                ?>
                <td><span class="badge bg-warning"><?php echo $days; ?> days left</span></td>
              <?php } else { ?>
